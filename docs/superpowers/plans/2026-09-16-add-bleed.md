# Add Bleed (version 1) – Implementation Plan
...
**Goal:** InDesign script that adds bleed to selected image frames by scaling or mirroring (spec: `docs/superpowers/specs/2026-09-16-add-bleed-design.md`).

**Architecture:** One ExtendScript file `Beschnitt_ergaenzen.jsx` with namespace `BE`: pure geometry functions (no DOM), DOM helpers, two methods, workflow/dialog. `BE.run()` is only executed when `$.global.BE_TEST` is not set. Tests run in InDesign 2026, started via AppleScript; the runner loads the script and all `test/test_*.jsx`, only works with its own hidden documents and writes `test/output/result.txt`.

**Tech Stack:** ExtendScript (ES3) for InDesign 2025/2026, ScriptUI, AppleScript (`osascript`), bash.

## Global Constraints

- One script file, ExtendScript/ES3 (no `let`, no arrow functions, no `Array.prototype.forEach`).
- Write non-ASCII characters in strings as `\uXXXX` (ExtendScript does not detect the file encoding reliably).
- Edge detection tolerance: 1 mm = `72 / 25.4` pt.
- Internally points + spread ruler origin; always restore settings in `finally`.
- Whole run = one undo step (`UndoModes.ENTIRE_SCRIPT`), named "Beschnitt ergänzen".
- Tests must never change open user documents; never get colours via localised swatch names.
- Test app: `Adobe InDesign 2026`.

## Files

- `Beschnitt_ergaenzen.jsx` – the script
- `test/run_tests.sh` – starts the runner in InDesign, prints the result, exit code 1 on failures
...
- `install.sh` – symlink into all Scripts Panel folders found
- `README.md`

---

### Task 1: Test infrastructure + pure geometry

**Files:**
- Create: `test/run_tests.sh`, `test/run_tests.jsx`, `test/test_1_geometrie.jsx`, `Beschnitt_ergaenzen.jsx`

**Interfaces:**
- Produces (bounds always `[top, left, bottom, right]` in pt):
  ...
  - `BE.scaleFactor(fb, gb, tb)` → number ≥ 1 or `null`
  - `BE.scaleBounds(b, cy, cx, s)` → bounds
  - `BE.mirrorPieces(fb, tb, edges)` → `[{flipH, flipV, axisX, axisY, rect}]`, order: top, bottom, left, right, corner t-l, t-r, b-l, b-r
...
- [ ] **Step 1: Write the runner** – `test/run_tests.sh`:
...
- [ ] **Step 2: Write the geometry tests** – `test/test_1_geometrie.jsx`:
...
Expected: exit 1 ("Keine Ergebnisdatei" or FAIL, because `Beschnitt_ergaenzen.jsx` is missing).

- [ ] **Step 4: Script skeleton + geometry** – `Beschnitt_ergaenzen.jsx`:
...
- [ ] **Step 5: Run the tests** – Run: `test/run_tests.sh` – Expected: `ERGEBNIS: … 0 fehlgeschlagen`, exit 0.

- [ ] **Step 6: Commit** – `git add -A && git commit -m "Geometrie und Test-Infrastruktur"`

---

### Task 2: Document helpers (bleed, settings, pages, frame check)

**Files:**
- Modify: `Beschnitt_ergaenzen.jsx` (new section before "Start")
- Create: `test/test_2_dokument.jsx`

**Interfaces:**
- Produces:
  - `BE.readBleed(doc)` → `{top,bottom,inside,outside}` (in the current script unit)
  - `BE.withSettings(doc, fn)` → return value of `fn`; sets points/spread origin/zero point 0, restores in `finally`
  - `BE.pageInfo(frame)` → `{page:[4], spreadLeft, spreadRight, leftSide, rightSide}`
  - `BE.isRectangular(frame)` → bool
  - `BE.resolveFrame(item)` → `{frame: PageItem|undefined, reason: String|null}`; exact reasons: `"kein Bildrahmen"`, `"Rahmen enthält keine Grafik"`, `"liegt in einer Gruppe oder ist verankert"`, `"gesperrt"`, `"liegt auf der Montagefläche"`, `"gedreht oder verzerrt"`, `"keine Rechteckform"`

- [ ] **Step 1: Write the tests** – `test/test_2_dokument.jsx`:
...
- [ ] **Step 2: Run the tests** – `test/run_tests.sh` – Expected: FAIL (`BE.readBleed` etc. undefined).

- [ ] **Step 3: Implement** – insert into `Beschnitt_ergaenzen.jsx` before `// ---------- Start ----------`:
...
- [ ] **Step 4: Run the tests** – `test/run_tests.sh` – Expected: 0 failed.
- [ ] **Step 5: Commit** – `git add -A && git commit -m "Dokument-Helfer und Rahmenpruefung"`

---

### Task 3: Scale method + processFrame
...
- Produces:
  - `BE.setFrameBounds(frame, b)` – set the frame, content stays (autoFit temporarily off)
  - `BE.applyScale(frame, fb, tb)` → `null` or reason
  - `BE.processFrame(frame, method, bleed)` → `null` or reason; `method` ∈ `"scale"|"mirror"`; reason when no edge is found, exactly `"liegt nicht am Seitenrand (oder hat schon Beschnitt)"`
  - `BE.applyMirror` is added in Task 4 (only called here)

- [ ] **Step 1: Write the tests** – `test/test_3_skalieren.jsx`:
...
- [ ] **Step 2: Run the tests** – Expected: FAIL (`BE.processFrame` undefined).

- [ ] **Step 3: Implement** – insert before "Start":
...
- [ ] **Step 4: Run the tests** – Expected: 0 failed.
- [ ] **Step 5: Commit** – `git add -A && git commit -m "Methode Skalieren"`

---

### Task 4: Mirror method
...
**Interfaces:**
- Consumes: `BE.mirrorPieces`, `t = {edges, target}` from `BE.computeTarget`
- Produces: `BE.applyMirror(frame, fb, t)` → `null`; creates a group of the original + pieces

- [ ] **Step 1: Write the tests** – `test/test_4_spiegeln.jsx`:
...
- [ ] **Step 2: Run the tests** – Expected: FAIL (`BE.applyMirror` undefined).

- [ ] **Step 3: Implement** – insert after `BE.applyScale`:
...
### Task 5: Workflow, dialog, undo, installation, README
...
- [ ] **Step 1: Write the tests** – `test/test_5_ablauf.jsx`:
...
- [ ] **Step 2: Run the tests** – Expected: FAIL (`BE.process` undefined).
...
- [ ] **Step 4: Run the tests** – Expected: 0 failed. If the undo test fails because `doScript` undo behaves differently for this document: find the cause with superpowers:systematic-debugging, do not weaken the test.

- [ ] **Step 5: install.sh and README**
...
- [ ] **Step 6: Install and check by hand** – `chmod +x install.sh && ./install.sh`. The user tests in InDesign: dialog, scale, mirror, undo, messages for missing selection/missing bleed.
